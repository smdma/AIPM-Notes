...
LeNet-5是一个CNN模型的开源组织，地址为http://yann.lecun.com/exdb/lenet/。
...
而卷积神经网络的特点在于隐藏层分为**卷积层**和**池化层**(pooling layer，又叫**下采样层**)以及**激活层**。每一层的作用：
...
卷积核的作用是参数的压缩，以此将复杂参数的训练流程简化，整个CNN学习的过程就是在确定卷积核的具体数值。
...
- max_polling：取池化窗口的最大值，好处是保留了数据中的最主要特征而且能减小计算复杂度。
...
通常池化层采用 **2x2大小、步长为2**窗口，其目的是将特征图下采样 2 倍。与此相对的是，卷积通常使用 3×3 窗口和步长1。 

使用下采样的原因，一是减少需要处理的特征图的元素个数，二是通过让连续卷积层的观察窗口越来越大（即窗口覆盖原始输入的比例越来越大），从而引入空间过滤器的层
级结构。 

## 5、全连接层(Full Connection)

经历了两次卷积和采样处理，生成了许多的特征图谱（Feature Map），这些特征图谱（Feature Map）可以看作对于输入图片的位置信息以及表现的描述，不同的特征图谱（Feature Map）会表示不同的内容，如某一个点的位置、字母中某一个模块的朝向等。

但是我们需要解决的场景是识别输入图片中的字母“A”，光有这些特征图谱（Feature Map）是不够的，因为特征图谱（Feature Map）描述的是输入图片的各种细节信息，需要把这些信息进行汇总并且综合判断，返回我们需要的结果。全连接以及高斯连接会做一个分类结果的判断，判断输入结果是否为“A”。
